...
// src/components/UpcyclePage.js - Updated with direct API integration
import React, { useState, useEffect, useRef } from 'react';
import Webcam from 'react-webcam';
import * as tf from '@tensorflow/tfjs';
import * as cocoSsd from '@tensorflow-models/coco-ssd';
...
function UpcyclePage() {
  ...
  // --- Direct API call to Google Gemini (client-side) ---
  const fetchAIIdeas = async (objectName) => {
    setProcessingImage(true);
    setIdentifiedObject(objectName);
    setDisplayIdeas([]);
    ...
    try {
      ...
      const API_KEY = 'AIzaSyDPaSd2wBdcG8X89qO0K1O4hO9yLH7VzvM'; // Your API key
      const API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${API_KEY}`;

      const prompt = `You are Upcyclr, a creative assistant for upcycling.
...
Do not include any other text, warnings, or explanations in your response, just the JSON.`;
      ...
      console.log("AI-generated ideas received:", ideas);

    } catch (error) {
      console.error("Could not fetch ideas from AI:", error);
      setDisplayIdeas([
        {
          title: "Failed to generate ideas.",
          description: "An error occurred while connecting to the AI. Please check your internet connection.",
          steps: ["Try refreshing the page", "Check your internet connection", "Try again in a moment"],
          links: []
        }
      ]);
    } finally {
  ...
  // --- Update your `runObjectDetection` function ---
  const runObjectDetection = async (imageSrcOrElement) => {
    if (!model) {
      console.log("AI model not yet loaded.");
      setIdentifiedObject("AI model is still loading. Please wait.");
  ...
  const capturePhoto = () => {
    if (webcamRef.current) {
      const imageSrc = webcamRef.current.getScreenshot();
      setWebcamImagePreviewUrl(imageSrc);
      setUploadedImagePreviewUrl(null);
      setShowWebcam(false);
      setAcceptedFeedback(false);
      runObjectDetection(imageSrc);
